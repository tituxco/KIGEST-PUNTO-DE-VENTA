Public Class selprod
    ...
    Public Sub CargarProductos()
        Try
            ...
            "(select if (isnull(stock),0,sum(replace(stock,',','.'))) from fact_insumos_lotes  
            where idproducto= ins.id and idalmacen=" & My.Settings.idAlmacen & ") as stock, codigo as COD, 
            round((replace(precio,',','.') * 1.21),2) as precioLista from fact_insumos as ins " _
            & " where " & busqtxt & " and eliminado=0 order by descripcion asc limit 30", GestorConexiones.conexionPrinc)
